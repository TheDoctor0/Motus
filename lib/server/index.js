...
const TransactionAssertionError = require('../blockchain/error/transactionAssertionError');
const BlockAssertionError = require('../blockchain/error/blockAssertionError');
...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.locals.formatters = {
            ...
            hash: (hashString) => {
                return hashString != '0' ? `${hashString.substr(0, 8)}...${hashString.substr(hashString.length - 8, 8)}` : '<empty>';
            },
        ...
        this.app.get('/blockchain', (req, res) => {
            if (req.headers['accept'] && req.headers['accept'].includes('text/html'))
                res.render('blockchain/index.pug', {
                    ...
                    blocks: blockchain.getAllBlocksDescending()
                });
        ...
        this.app.put('/blockchain/blocks/latest', (req, res) => {
            ...
            node.connectToPeer({ url: 'http://' + req.headers.host });
        ...
        this.app.post('/operator/wallets', (req, res) => {
            ...
            if (R.match(/\w+/g, password).length < 5) throw new HTTPError(400, 'Password must contain at least 5 words');
        ...
        this.app.post('/miner/mine', (req, res, next) => {
            let rewardAddress = req.body.rewardAddress;

            if (rewardAddress == null) throw new HTTPError(401, 'Rewards address is missing.');
    ...
    listen(host, port) {
        return new Promise((resolve, reject) => {
            this.server = this.app.listen(port, host, (err) => {
                ...
                console.info(`Listening on http://${host}:${this.server.address().port}`);
